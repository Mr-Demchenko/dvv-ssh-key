...
- поиск операций по описанию с помощью регулярных выражений;
- подсчёт количества операций по категориям;
- консольный сценарий работы с транзакциями в модуле `main.py`;
...
category_counts = process_bank_operations(
    transactions,
    ["Перевод организации", "Открытие вклада"],
)
```

Функция `process_bank_search` ищет операции по полю `description` с использованием библиотеки `re`.
Функция `process_bank_operations` считает количество операций по категориям с использованием `Counter`.

### Консольный сценарий

Основная пользовательская логика находится в модуле `main.py`.
...
Программа позволяет выбрать источник данных, отфильтровать операции по статусу, отсортировать их по дате, оставить только рублёвые операции и выполнить поиск по описанию.
